...
const tool = {
  ...
  handler: async (args: Record<string, unknown>, signal: AbortSignal) => {
    ...
    const schemaArg = (args.input_schema && typeof args.input_schema === 'object') ? args.input_schema as Record<string, unknown> : undefined;
    const handlerCodeArg = typeof args.handler_code === 'string' ? args.handler_code : undefined;
    const reqPkgsArg = (args.required_packages && typeof args.required_packages === 'object') ? args.required_packages as Record<string, string> : undefined;
    ...
    if (!finalName || !finalName.startsWith('generated_') || (baseNameArg && !isValidBaseName(baseNameArg))) {
      return { content: [{ type: 'text', text: 'Provide tool_name (generated_*) or valid base_name.' }], isError: true };
    }
    ...
    else return { content: [{ type: 'text', text: 'index.ts/js not found for tool' }], isError: true };

    const source = await readFile(filePath, 'utf8');
    const isGenerated = GENERATED_HEADER_MARKERS.some(m => source.includes(m));
    if (!isGenerated) {
      return { content: [{ type: 'text', text: 'Refusing to edit: not recognized as a generated tool.' }], isError: true };
    }
    ...
    if (handlerCodeArg) {
      ...
      if ((issues.blocking.length > 0 || issues.warnings.length > 0) && !allowUnsafe) {
        ...
        return { content: [{ type: 'text', text: `Unsafe constructs detected. Aborting. ${msg}` }], isError: true };
      }
    }
    ...
    return { content: [{ type: 'text', text: lines || `Edited ${finalName}` }] };
  }
};

...
async function updateDependencies(projectRoot: string, deps: StringMap | undefined) {
  ...
  const pkg = JSON.parse(raw) as { dependencies?: Record<string, string>; [key: string]: unknown };
  pkg.dependencies ||= {};
  const added: string[] = [];
...
const tool = {
  ...
  handler: async (args: Record<string, unknown>, signal: AbortSignal) => {
    ...
    const baseName = String(args['base_name'] ?? '');
    const desc = String(args['description'] ?? '');
    const inputSchema = args['input_schema'];
    const handlerCode = typeof args['handler_code'] === 'string' ? (args['handler_code'] as string) : undefined;
    const template = typeof args['template'] === 'string' ? (args['template'] as string) : undefined;
    const requiredPackages = (args['required_packages'] && typeof args['required_packages'] === 'object')
      ? (args['required_packages'] as Record<string, string>)
      : undefined;
...
`;
    ...
    try {
      ...
      if (refresh?.handler) {
        await refresh.handler({ plugin: finalName } as Record<string, unknown>, signal);
        refreshOk = true;
      } else {
